...
module.exports = function(server) {
  ...
  server.get('/users/:userId/todos', (req, res, next) => {
    ...
    Todo.find(query).skip(skip).limit(limit).then(tasks => {
      res.send(200, tasks)
      next()
    }).catch(err => {
      res.send(500, err)
  ...
  server.get('/users/:userId/todos/:todoId', (req, res, next) => {
    ...
    Todo.findOne({userId: req.params.userId, _id: req.params.todoId}).then(todo => {
      res.send(200, todo)
      next()
    }).catch(err => {
      res.send(500, err)
  ...
  server.del('/users/:userId/todos/:todoId', (req, res, next) => {
    ...
    Todo.findOneAndRemove({userId: req.params.userId, _id: req.params.todoId}).then(() => {
      res.send(204)
      next()
    }).catch(err => {
      res.send(500, err)
